...
int main() {
    ...
    /*
    // Create a ground body (static) to act as a surface
    b2BodyDef groundBodyDef;
    groundBodyDef.position.Set(0.0f, -10.0f);
    b2Body* groundBody = world.CreateBody(&groundBodyDef);

    b2PolygonShape groundBox;
    groundBox.SetAsBox(50.0f, 10.0f); // A large horizontal box to act as the ground
    ...
    /*
     * // setting bullet's mass
    b2MassData massData;
    massData.mass = 0.0650f;
    massData.center = bulletBody->GetPosition();
    massData.I = 0.1f;
    bulletBody->SetMassData(&massData);
    float mass = massData.mass;
    std::cout << "Bullet Mass: " << mass << " kg\n";
     */
